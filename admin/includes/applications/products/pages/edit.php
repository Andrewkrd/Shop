  ...
  function showImages(data) {
    for ( i=0; i<data.entries.length; i++ ) {
      ...
      if ( entry[1] == '1' ) { // original (products_images_groups_id)
        var onmouseover = 'this.style.backgroundColor=\'#EFEBDE\'; this.style.backgroundImage=\'url(<?php echo osc_href_link_admin('templates/default/images/icons/16x16/drag.png'); ?>)\'; this.style.backgroundRepeat=\'no-repeat\'; this.style.backgroundPosition=\'0 0\';';
...
<?php
if ( isset($osC_ObjectInfo) ) {
//org echo '<input type="button" id="uploadFile" value="' . $osC_Language->get('button_send_to_server') . '" class="operationButton" /><div id="showProgress" style="display: none; padding-left: 10px;">' . osc_icon('progress_ani.gif') . '&nbsp;' . $osC_Language->get('image_upload_progress') . '</div>';
echo '<input id="products_image" type="file" name="products_image">';
echo '<button id="buttonUpload" onClick="return ajaxFileUpload();" class="operationButton">' . $osC_Language->get('button_send_to_server') . '</button>';
echo '<div id="showProgress" style="display: none; padding-left: 10px;">' . osc_icon('progress_ani.gif') . '&nbsp;' . $osC_Language->get('image_upload_progress') . '</div>';
    } else {
...
<script type="text/javascript">
	function ajaxFileUpload()
	{
		...
		(
			{
				url:'<?php echo osc_href_link_admin('rpc.php', $osC_Template->getModule() . '=' . $osC_ObjectInfo->getInt('products_id') . '&action=fileUpload'); ?>',
				secureuri:false,
				fileElementId:'products_image',
				dataType: 'json',
...
<?php
    if ( isset($osC_ObjectInfo) ) {
      echo '<input type="button" value='. $osC_Language->get('assign_to_product') .' class="operationButton" onclick="assignLocalImages();" /><div id="showProgressAssigningLocalImages" style="display: none; padding-left: 10px;">' . osc_icon('progress_ani.gif') . '&nbsp;' . $osC_Language->get('image_multiple_upload_progress') . '</div>';
    }
?>
...
?>
  ...
  <div id="section_variants_content">
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td colspan="3" align="right"><input type="button" value= <?php echo $osC_Language->get('access_configuration_add') ?> class="infoBoxButton" onclick="addVariant();" /></td>
      </tr>
      <tr>
...
<?php
  if ( $has_multiple_value_groups === true ) {
    echo '<div style="text-align: center; font-style: italic;"></div>';
  }
?>
