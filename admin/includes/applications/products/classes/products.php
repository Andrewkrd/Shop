...
//              $Qp2c->setLogging($_SESSION['module'], $products_id);
      ...
      if ( $error === false ) {
        ...
        foreach ($images as $tmp_image) {
          ...
          $Qimage->bindValue(':image', $tmp_image);
          ...
          if ( $osC_Database->isError() ) {
            ...
            foreach ($osC_Image->getGroups() as $group) {
              if ($group['id'] != '1') {
                $osC_Image->resize($tmp_image, $group['id']);
              }
            }
          }
        }
...
//          $Qpd->setLogging($_SESSION['module'], $products_id);
      ...
      if ( $error === false ) {
		...
		$Qattribute = $osC_Database->query('delete from :table_product_attributes where value = "NONE"');
        $Qattribute->bindTable(':table_product_attributes', TABLE_PRODUCT_ATTRIBUTES);
        $Qattribute->execute();			
