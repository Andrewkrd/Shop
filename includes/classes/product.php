  ...
  class osC_Product {
    ...
    function getPriceFormated($with_special = false) {
      ...
      if (($with_special === true) && $osC_Services->isStarted('specials') && ($new_price = $osC_Specials->getPrice($this->_data['id']))) {
        $price = '<s>' . $osC_Currencies->displayPrice($this->_data['price'], $this->_data['tax_class_id']) . '</s><br /><span class="productSpecialPrice">' . $osC_Currencies->displayPrice($new_price, $this->_data['tax_class_id']) . '</span>';
      } else {
        if ( $this->hasVariants() ) {
          $price =  $osC_Language->get('variants_tag') . '&nbsp;' . $this->getVariantMinPrice();
        } else {
    ...
    global $osC_Currencies;
      ...
      foreach ( $this->_data['variants'] as $variant ) {
        if ( ($price === null) || ($variant['data']['price'] < $price) ) {
          ...
          $tax = $variant['data']['tax_class_id'];
        }
      }
$price = $osC_Currencies->displayPrice($price, $tax);
