 ...
 */
    ...
    public static function createEntry($data) {
	  ...
	  $Qcustomer->bindValue(':customers_dob', ((ACCOUNT_DATE_OF_BIRTH == '-1') ? '1949-02-17 00:00:00' : ''));
	  }
	        if (ACCOUNT_DATE_OF_BIRTH == '1'){
...
//START TEXT-HTML E-MAIL	
	  if (EMAIL_USE_HTML == '1') {
        if ( (ACCOUNT_GENDER > -1) && isset($data['gender']) ) {
           if ( $data['gender'] == 'm' ) {
             $email_text = sprintf($osC_Language->get('email_addressing_gender_male'), $osC_Customer->getLastName()) . "<br><br>";
           } else {
             $email_text = sprintf($osC_Language->get('email_addressing_gender_female'), $osC_Customer->getLastName()) . "<br><br>";
           }
        } else {
          $email_text = sprintf($osC_Language->get('email_addressing_gender_unknown'), $osC_Customer->getName()) . "<br><br>";
        }

        $email_text .= nl2br(sprintf($osC_Language->get('email_create_account_body'), STORE_NAME, STORE_OWNER_EMAIL_ADDRESS));
