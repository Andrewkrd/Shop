...
/*  better ISO/UTF-8 mail 2.0 contains
    ...
    other corrections of better ISO/UTF-8 mail 2.0 contrib in
        ...
        admin\includes\modules\newsletters\email.php
        admin\includes\modules\newsletters\newsletter.php
        admin\includes\modules\newsletters\products_notification.php
...
*/
  ...
  class osC_Mail {
    var $_to = array(),
        ...
        $_charset = 'iso-8859-1'; //this is the base core charset of osc3.05alfa version 
    ...
    function osC_Mail($to = null, $to_email_address = null, $from = null, $from_email_address = null, $subject = null) {
      ...
      if ( !empty($subject) ) {
	    $this->_subject = "=?"  . $osC_Language->getCharacterSet() . '?B?' . base64_encode($subject) . "?=";
      }
    }
    ...
    function send() {
	global $osC_Language;
	$this->_charset = $osC_Language->getCharacterSet(); //redefined osC3.05alfa core language charset to global
      if ( empty($this->_body) ) {
...
/* set correct header of plain text */
		  $content_type = 'text/plain; charset="' . $this->_charset . '"';
		  if (EMAIL_USE_HTML == '1') {$content_type = 'text/html; charset="' . $this->_charset . '"';}	  
    ...
    function _name_encode($e_name) {
      global $osC_Language;
      return '=?' . $osC_Language->getCharacterSet() . '?Q?' .
      str_replace("+", "_", str_replace("%", "=", urlencode(quoted_printable_decode($e_name)))) . '?= ';  // Quoted-printable string
//    return '=?' . $osC_Language->getCharacterSet() . '?B?' . base64_encode($e_name) . '?= ';   //Base64 encoding
//    return $e_name;   //plain/text  not recomended
    }
