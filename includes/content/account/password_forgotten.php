...
//START TEXT-HTML E-MAIL	
	  if (EMAIL_USE_HTML == '1') {		
		...
		if ($data['customers_gender'] == 'm') {
               $email_text = nl2br(sprintf($osC_Language->get('email_addressing_gender_male'), $Qcheck->valueProtected('customers_lastname'))) . "<br><br>";
             } else {
               $email_text = nl2br(sprintf($osC_Language->get('email_addressing_gender_female'), $Qcheck->valueProtected('customers_lastname'))) . "<br><br>";
             }
          } else {
            $email_text = nl2br(sprintf($osC_Language->get('email_addressing_gender_unknown'), $Qcheck->valueProtected('customers_firstname') . ' ' . $Qcheck->valueProtected('customers_lastname'))) . "<br><br>";
          }

          $email_text .= nl2br(sprintf($osC_Language->get('email_password_reminder_body'), getenv('REMOTE_ADDR'), STORE_NAME, $password, STORE_OWNER_EMAIL_ADDRESS));
