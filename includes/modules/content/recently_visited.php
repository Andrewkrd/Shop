      ...
      if ($osC_Services->isStarted('recently_visited') && $osC_RecentlyVisited->hasHistory()) {
        ...
        if ($osC_RecentlyVisited->hasSearches()) {
          ...
          foreach ($osC_RecentlyVisited->getSearches() as $searchphrase) {
            $this->_content .= '<li>' . osc_link_object(osc_href_link(FILENAME_SEARCH, 'keywords=' . $searchphrase['keywords']), osc_output_string_protected($searchphrase['keywords'])) . ' <i>(' . number_format($searchphrase['results']) . ' ' . $osC_Language->get('recently_visited_searches_results') . ')</i></li>';
          }
