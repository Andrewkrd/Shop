    ...
    function initialize() {
      ...
      if ($osC_Services->isStarted('reviews')) {
        ...
        if (empty($data)) {
          ...
          if (!empty($data['image'])) {
            $this->_content = '<div align="center">' . osc_link_object(osc_href_link(FILENAME_PRODUCTS, 'reviews=' . $data['reviews_id'] . '&' . $data['products_keyword']), $osC_Image->show($data['image'], $data['products_name'])) . '</div><div align="center">';
          }

          $this->_content .= osc_link_object(osc_href_link(FILENAME_PRODUCTS, 'reviews=' . $data['reviews_id'] . '&' . $data['products_keyword']), utf8_wordwrap(osc_output_string_protected($data['reviews_text']), 25) . '...') . '</div><br /><div align="center">' . osc_image('templates/' . $osC_Template->getCode() . '/images/stars_' . $data['reviews_rating'] . '.png' , sprintf($osC_Language->get('box_reviews_stars_rating'), $data['reviews_rating'])) . '</div>';
        }
      }
    }
