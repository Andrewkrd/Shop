    ...
    while ($Qlisting->next()) {
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_BUY_NOW':
            $lc_align = 'center';
            $lc_text = osc_link_object(osc_href_link(basename($_SERVER['SCRIPT_FILENAME']), $osC_Product->getKeyword() . '&' . osc_get_all_get_params(array('action')) . '&action=cart_add'), $osC_Template->osc_draw_image_jquery_button(array('icon' => 'cart', 'title' => $osC_Language->get('button_buy_now')))) . '&nbsp;';
            break;
        }

        echo '      <td ' . ((empty($lc_align) === false) ? 'align="' . $lc_align . '" ' : '') . 'id="FormButtons" class="productListing-data">' . $lc_text . '</td>' . "\n";
      }
